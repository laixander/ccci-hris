...
<template>
	<div ref="container" class="flex flex-col flex-1 overflow-y-auto scrollbar">
			...
			<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4">
				<UPageCard title="Timesheet Management" description="Review and evaluate your daily time records."
					...
					<div class="flex justify-end gap-2 flex-1">
						<UFieldGroup>
							<UButton icon="i-lucide-calendar" color="neutral"
								...
								<template v-if="cell.record">
									...
									<div class="mt-auto pt-1 flex items-center justify-between">
										...
										<div class="opacity-0 group-hover:opacity-100 transition-opacity">
											<UButton icon="i-lucide-edit-3" size="xs" color="primary" variant="ghost"
								...
								<template v-if="cell.record">
									...
									<div class="flex flex-col gap-2">
										<div class="flex justify-between items-center text-sm">
											...
											<span class="font-medium text-toned">{{ cell.record.duration > 0 ?
												cell.record.duration.toFixed(2)
												+ 'h' : '-' }}</span>
										</div>
										<div class="flex justify-between items-center text-sm"
											v-if="cell.record.overtime > 0">
											<span class="text-dimmed">Overtime</span>
											<span class="font-medium text-warning">{{ cell.record.overtime.toFixed(2) +
												'h' }}</span>
										</div>
									</div>
									...
									<div class="mt-auto pt-4 flex items-center justify-between">
										<StatusBadge :status="cell.record.status" />
										<div class="opacity-0 group-hover:opacity-100 transition-opacity">
											<UButton icon="i-lucide-edit-3" size="sm" color="primary" variant="ghost"
												@click.stop="selectedRecord = cell.record!; isModalOpen = true" />
										</div>
									</div>
								</template>
		...
		<!-- Table view ────────────────────────────────────────────────────────── -->
		<UTable v-else :data="tableData" :columns="columns" sticky class="flex-1"
			:virtualize="{ scrollMargin: headerHeight, getScrollElement }">
			<template #empty>
				<UEmpty icon="i-lucide-clock" title="No time records"
					description="There are no daily time records available for this period." variant="naked" />
			</template>
		</UTable>
	...
	<UDrawer v-model:open="isDrawerOpen" direction="right" title="Time Adjustments" inset close
					...
					<div class="divide-y divide-[var(--ui-border)]">
						<div v-for="(entry, index) in timeAdjustmentData" :key="index" class="p-4 space-y-4">
							<div class="flex justify-between items-start">
								<div>
									<div class="text-sm font-semibold text-highlighted">Adjustment for {{ entry.date }}
									</div>
							...
							<div class="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<div class="flex items-center gap-2 text-sm">
										<div
											class="flex items-center gap-1.5 bg-primary/10 px-2 py-1 rounded-md text-primary">
	...
	<ApplyTimeAdjustmentModal v-model:open="isModalOpen" :record="selectedRecord"
		:month-label="months.find(m => m.value === month)?.label" :year="year" />
</template>