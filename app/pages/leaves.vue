...
<template>
	<div ref="container" class="flex flex-col flex-1 overflow-y-auto scrollbar">
		<div ref="header">
			<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4">
				<UPageCard title="Leave Management" description="Request and manage your leaves" variant="naked"
			...
			<div v-if="viewMode !== 'month'" class="flex items-center gap-3 px-4 pb-4">
				<UInput v-model="search" placeholder="Search by type or status..." icon="i-lucide-search"
					class="flex-1" />
			...
			<div v-if="viewMode !== 'month' && viewStats" class="flex gap-3 px-4 pb-4">
		...
		<div v-if="viewMode === 'grid'" class="flex-1 flex flex-col p-4">
			<div v-if="filteredLeaves.length === 0" class="flex-1 flex items-center justify-center">
				<UEmpty icon="i-lucide-calendar-x" title="No leave requests"
					description="No leave records found for the selected period." variant="naked" />
			</div>
			<div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
				<UCard v-for="leave in filteredLeaves" :key="leave.id" class="flex flex-col shadow-sm"
					...
					<div class="mt-auto flex items-center justify-between">
						...
						<UAvatarGroup :max="4" size="sm">
							<UAvatar v-for="(approver, index) in leave.approvers" :key="index" :src="approver.src"
								:chip="{ inset: true, color: 'success', position: 'bottom-right', size: 'md' }" />
						</UAvatarGroup>
					</div>
				</UCard>
			</div>
			...
			<div class="flex-1 flex flex-col overflow-x-auto scrollbar">
				<div class="flex flex-col flex-1 min-w-[800px]">
					...
					<div class="grid grid-cols-7 gap-px bg-[var(--ui-border)] flex-1 auto-rows-fr">
						<div v-for="(cell, idx) in calendarDays" :key="idx" class="p-2 flex flex-col bg-[var(--ui-bg)]">
							<template v-if="cell">
								...
								<div class="flex flex-col gap-0.5 mt-0.5">
									<UPopover v-for="(leave, li) in cell.leaves.slice(0, 2)" :key="li" mode="click"
										:content="{ align: 'start', side: 'right' }">
										<UBadge :label="leave.type" variant="soft" size="sm"
											class="w-full truncate cursor-pointer text-[10px]"
											:class="getLeaveColor(leave.type)" :ui="{ base: 'justify-start' }" />
										<template #content>
											<div class="p-3 w-auto space-y-2.5">
												...
												<div class="grid grid-cols-2 gap-2 text-xs bg-muted/50 rounded-md p-2">
													<div>
														...
														<div class="font-medium text-toned whitespace-nowrap">{{
															leave.start }}</div>
													</div>
													<div>
									...
									<div v-if="cell.leaves.length > 2" class="text-[10px] text-dimmed px-1 font-medium">
		...
		<UTable v-else :data="filteredLeaves" :columns="columns" sticky class="flex-1 min-h-0"
			...
			<template #approvers-cell="{ row }">
				<UAvatarGroup :max="4" size="sm">
					<UAvatar v-for="(approver, index) in row.original.approvers" :key="index" :src="approver.src"
						:chip="{ inset: true, color: 'success', position: 'bottom-right', size: 'md' }" />
				</UAvatarGroup>
			</template>
			<template #empty>
	...
	<UDrawer v-model:open="isDrawerOpen" direction="right" title="Leave Ledger" inset close class="w-full max-w-[460px]"
			...
			<div class="flex-1 min-h-0 overflow-y-auto p-[1px] scrollbar">
				<UCard class="shadow-sm" :ui="{ body: 'p-0 sm:p-0' }">
					<div class="divide-y divide-[var(--ui-border)]">
						<div v-for="(entry, index) in ledgerData" :key="index"
							class="flex items-center gap-3 p-4 group">
							<!-- Credit / Debit indicator -->
							<div class="shrink-0 size-9 rounded-full flex items-center justify-center"
							...
							<div class="text-right shrink-0">
								<div class="text-sm font-semibold tabular-nums"
									:class="entry.credit !== null ? 'text-success-500' : 'text-error-500'">
									{{ entry.credit !== null ? `+${entry.credit.toFixed(2)}` : `${(entry.debit ??
										0).toFixed(2)}` }}
								</div>
								<div class="text-xs text-dimmed tabular-nums">
									bal. <span class="text-highlighted font-medium">{{ entry.balance.toFixed(2)
										}}</span>
								</div>
							</div>

							<!-- Avatar -->
							<UAvatar :src="entry.avatar" :alt="entry.entryBy" size="xs"
